...
export default async function AdminModelsPage() {
  ...
  const overrides = settings?.modelOverrides ? JSON.stringify(settings.modelOverrides, null, 2) : '';
    ...
    <div className="mx-auto max-w-3xl p-2 md:p-4 space-y-6">
      ...
      <div className="rounded-xl border p-4 space-y-3">
        ...
        <form method="post" action="/admin/api/settings" className="space-y-2">
          <input type="hidden" name="key" value="reasoningRequiresSubscription" />
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" name="value" value="true" defaultChecked={requires} />
      ...
      <div className="rounded-xl border p-4 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">Optional JSON mapping to override underlying provider model IDs.</p>
        ...
        <form method="post" action="/admin/api/settings" className="space-y-2">
          ...
          <textarea name="value" defaultValue={overrides} className="w-full h-48 border rounded px-2 py-1 text-sm font-mono" />
          <button className="border rounded px-3 py-1 text-sm hover:bg-muted" type="submit">Save</button>
        </form>
      </div>
    </div>
